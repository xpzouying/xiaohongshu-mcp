...
| Issue | 标题 | 严重性 | 验证结果 | 状态 |
|-------|------|--------|---------|------|
| #470 | Chrome 进程泄漏 | **高** | 3 次 API 调用后产生 11 个 Chrome 进程，30 秒后未回收 | **已修复** (commit df8ff2a) |
| #501 | 发布时重复发送 | **高** | 代码审查确认：发布按钮点击后无防重复机制，缺少状态检查 | **已修复** (commit df8ff2a) |
| #511 | 选择原创时发布失败 | **高** | 代码审查确认：JS `checkbox.click()` 不触发 React/Vue change 事件，按钮保持 disabled | **已修复** (commit df8ff2a) |
| #569 | 搜索返回 displayTitle 为空 | **中** | 22 条搜索结果全部 displayTitle 和 user 信息为空 | 待修复 |
| #578 | 评论加载不完整 | **中** | load_all_comments 请求直接超时，无法完成全量加载 | 待修复 |
| #533/#504 | reply_comment_in_feed 评论定位失败 | **高** | 代码审查确认：选择器 `#comment-{id}` 和 `.parent-comment` 可能不匹配当前小红书页面结构 | 待修复 |
...
| #470 | Chrome 进程泄漏 | 复用单个浏览器实例，不再每次请求创建新 Chrome（commit df8ff2a） |
| #501 | 发布时重复发送 | 发布前检查按钮 disabled 状态，点击后等待确认（commit df8ff2a） |
| #511 | 选择原创时发布失败 | checkbox 改用 dispatchEvent 触发事件 + 重试机制（commit df8ff2a） |
